...
其中 point 运算由 :mod:`ascend.world.kernel.fixed` 提供（半偶舍入），
包含关系由 ``tests/world/test_kernel_enclosure.py`` 采样验证。定义域外/除零等未定义
...
from ascend.world.kernel.fixed import div, mul, round_half_even_div
...
def cos_enclosure(box: Interval) -> Interval:
    ...
    from ascend.world.kernel.frozen_tables import PI_Q, TABLE_BITS, TWO_PI_Q
...
def sin_enclosure(box: Interval) -> Interval:
    ...
    from ascend.world.kernel.frozen_tables import HALF_PI_Q, TABLE_BITS, TWO_PI_Q
