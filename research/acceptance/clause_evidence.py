...
CLAUSES: tuple[ClauseEvidence, ...] = (
    ...
    ClauseEvidence(
        ...
        positive="程序身份 = 契约 + 模块摘要 + 参数 + 旋钮 + 调度 + 内核；世界身份 = 程序身份 + 种子",
        negative="改任一模块/参数 → 身份变（编译复现性测试）",
    ),
    ClauseEvidence(
        "WC-1.2", "covered",
        checks=("C0",),
    ...
    ClauseEvidence(
        ...
        evidence=("backend/tests/world/test_dynamic_instances.py",
                  "backend/tests/world/test_instances.py"),
        positive="物化无关：多 chunk / 不同物化序逐位一致",
    ...
    ClauseEvidence(
        "WC-5.7", "partial",
        evidence=("backend/ascend/weather/field.py",
                  "backend/ascend/weather/atmosphere.py"),
        positive="天气场/特征核噪声经地址随机派生（结构化地址，P2-3a）",
        negative="裸 seed 偏移在门禁外（代码审查约定）",
        note="噪声场的批量接口与声明化随 P5 生成程序声明化；当前实现为解析算（seed + 坐标 + 时间）。",
    ),
    ClauseEvidence(
        "WC-5.8", "covered",
        checks=("L3-历史",),
