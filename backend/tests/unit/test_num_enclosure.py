...
from ascend.num.fixed import div as point_div
from ascend.num.fixed import mul as point_mul
...
class TestTableEnclosures:
    ...
    def test_cos_and_sin_inclusion(self):
        ...
        for _ in range(1500):
            ...
            for _ in range(4):
                x = rng.randint(lo, hi)
                from ascend.num.tables import cos_q as point_cos
                from ascend.num.tables import sin_q as point_sin
                assert cbox.contains(point_cos(x)), (box, x, cbox)
                assert sbox.contains(point_sin(x)), (box, x, sbox)

    def test_cos_extrema_detected(self):
        scale = 1 << 30
        from ascend.num.frozen_tables import PI_Q
    ...
    def test_tanh_and_acos_and_degrees_inclusion(self):
        ...
        for _ in range(800):
            ...
            for _ in range(4):
                x = rng.randint(lo, hi)
                from ascend.num.tables import degrees_q as point_deg
                from ascend.num.tables import tanh_q as point_tanh
    ...
    def test_tan_pole_rejected(self):
        scale = 1 << 30
        from ascend.num.frozen_tables import HALF_PI_Q
