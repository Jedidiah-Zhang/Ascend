...
版本**（声明 ID + 全量摘要）、**观测协议版本**，以及**世界程序身份**
（声明编译产物摘要：槽位/地址/更新点/日历刻度的组合）。前两者随存档位
创建时定案，其余由当前进程的编译产物提供。

读档时若记录与当前进程不一致，说明这个世界的生成规律已经变了：用新
公式继续跑旧状态会得到一条"合法但不属于任何已声明世界"的轨迹
（静默错误）。本模块只做一件事——把这种不一致挡在加载之前。

模块边界：本模块不认识注册表，也不认识编译器，只比较调用方传入的
**声明视图**（``MechanismRegistry.declaration_settings()``）与
**程序视图**（``WorldProgram.settings()``），因此存档层不依赖因果层。
"""
