...
校验以**编译后的世界程序**（``WorldProgram``）为唯一事实源：目标槽位
是否存在且由机制写入、槽位权限/值域/实例域、参数是否被已接线机制消费
——不读任何运行时全局注册表。时间线绑定哪个程序，就只接受该程序的
求值面（引擎绑定 wired 子集，研究绑定完整切片）。
...
from ascend.world.meta.declarations import ParameterDecl, SlotDecl
...
class InterventionTimeline:
    ...
    Parameters:
        program: 编译后的世界程序（提供槽位声明/权限/值域/实例域、机制
            消费的参数集）；时间线只接受该程序的求值面。
        now: 世界时钟读取函数 ``() -> tick``；登记时盖章 ``submitted_at``。
            None = 不记录登记时刻（仅测试/无世界句柄场景）。
        instance_exists: 实例存在性查询 ``(槽位 ID, 实例元组) -> bool``；
    ...
    def __init__(
        ...
        *,
        ...
        self._parameter_decls: dict[str, ParameterDecl] = {}
        for pack in program.modules:
            for declaration in pack.parameters:
                self._parameter_decls.setdefault(declaration.id, declaration)
        consumed: set[str] = set()
        for mechanism in program.mechanisms.values():
            consumed.update(mechanism.params)
        self._consumed_parameters = tuple(sorted(consumed))
    ...
    @property
    def program(self) -> WorldProgram:
        """绑定的不可变世界程序（供终端/研究 API 解析目标）。"""
        return self._program

    @property
    def consumed_parameters(self) -> tuple[str, ...]:
        """被已接线机制实际消费的参数（可干预的参数面）。"""
        return self._consumed_parameters

    def parameter_decl(self, parameter_id: str) -> ParameterDecl | None:
        """参数声明查询（终端/研究 API 的值解析用；未声明返回 None）。"""
        return self._parameter_decls.get(parameter_id)
    ...
    def _validate_node(self, entry: PlannedIntervention) -> None:
        slot = self._program.slots.get(entry.target)
        if slot is None:
        ...
        self._validate_instance(entry.target, entry.instance, slot)
        required = (
            "node" if entry.duration_class() == SINGLE else "persistent"
        )
        if required not in slot.access_interventions:
            raise ValueError(
                f"{entry.target} 不允许 {required} 干预 "
                f"(权限={slot.access_interventions})"
            )
        if entry.value is None:
            raise ValueError("值干预必须提供替换值")
        if not slot.domain.accepts(entry.value):
            raise ValueError(
                f"{entry.target}={entry.value!r} 不属于声明值域 "
                f"{slot.domain.kind}"
                + (
                    f" {list(slot.domain.choices)}"
                    if slot.domain.kind == "enum" else ""
                )
            )

    def _validate_instance(
        self, target: str, instance: tuple, slot: SlotDecl,
    ) -> None:
        domain = self._program.instances[slot.on]
        if domain.kind == "global":
            if instance != ():
                raise ValueError(
            ...
            return
        if not instance:
            raise ValueError(f"{target} 实例坐标不能为空: {instance}")
        if not all(
            isinstance(axis, int) and not isinstance(axis, bool)
            for axis in instance
        ):
            raise ValueError(f"{target} 实例坐标必须为整数: {instance}")
        axes = len(domain.axes) if domain.axes else len(domain.size or ())
        if len(instance) != axes:
            raise ValueError(
                f"{target} 实例必须匹配实例域 {domain.id} "
                f"axes={domain.axes or domain.size}: {instance}"
            )
        if (
    ...
    def _validate_parameter(self, entry: PlannedIntervention) -> None:
        declaration = self._parameter_decls.get(entry.target)
        if declaration is None:
            raise ValueError(f"目标参数未声明: {entry.target}")
        if entry.target not in self._consumed_parameters:
            raise ValueError(
                f"目标参数未被已接线机制消费（环境变化不会生效）: "
                f"{entry.target}"
            )
        if not declaration.intervention_allowed:
            raise ValueError(f"参数不允许干预（环境变化）: {entry.target}")
        if entry.value is None:
            raise ValueError("参数干预必须提供替换值")
        if not declaration.accepts(entry.value):
            raise ValueError(
                f"{entry.target}={entry.value!r} 不属于声明值域 "
                f"{declaration.kind}"
                + (
                    f" {list(declaration.choices)}"
                    if declaration.kind == "enum" else ""
                )
            )
