"""守恒练兵切片 — 流量 / 守恒不变量 / 多分辨率（P4-2）。

一个"不改框架"的完整模块：两级 lattice（流域 → 地块）+ 逐帧流量机制 +
跨槽位守恒不变量。用于演练装载位与模块接入协议：
...
MODULE = ModulePack(
    ...
    evidence=(
        "黄金语义：tests/world/test_conservation.py",
        "验收判据：W6（守恒切片）",
    ),
    notes="守恒练兵切片：流量 + 守恒不变量 + 多分辨率（P4-2）。",
)
