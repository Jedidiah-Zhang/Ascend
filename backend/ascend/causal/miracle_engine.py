"""神迹引擎 — 覆盖感知求值器与引擎级逐帧执行器。

``MiracleEvaluator`` 是生产求值入口：包裹注册表与神迹表，按 (目标,
实例, 帧) 解析值/机制神迹并在求值点替换；参数神迹经 ``parameter_values``
注入注册表求值（同一实现，见 registry.evaluate_mechanism）。

``MiracleFrameExecutor`` 是引擎级（研究切片）逐帧求值器：按微步序对
注册表切片逐节点求值，神迹覆盖生效，并维护 CRN 随机流契约（值覆盖
...
from .miracle import MiracleRecord, MiracleTable, NodeResolution
from .registry import MechanismRegistry


class MiracleEvaluator:
    """覆盖感知求值器（生产接线入口）。

    Parameters:
        registry: 不可变机制注册表。
        table: 神迹表；None = 直通注册表（零开销回退）。
    """

    def __init__(
        self,
        registry: MechanismRegistry,
        table: MiracleTable | None = None,
    ) -> None:
        self._registry = registry
        self._table = table

    @property
    def table(self) -> MiracleTable | None:
        return self._table
    ...
    def evaluate(
        ...
        *,
        ...
        Args:
            ...
            parameter_values: 参数槽位覆盖（神迹表内联解析）。
        """
        ...
        resolution = self._table.resolve_node(target, instance, frame)
        if resolution.rep == "value":
            return resolution.value
        merged_parameters = self._parameter_overrides(
            target, resolution, frame, parameter_values
        )
    ...
    def _parameter_overrides(
        ...
        """合并调用方参数覆盖与神迹表参数神迹（表内活跃覆盖优先）。"""
        if self._table is None:
            return dict(parameter_values) if parameter_values else None
        mechanism = None
        if resolution.rep == "mechanism":
        ...
        for binding in mechanism.parameters:
            active, value = self._table.resolve_parameter(
                binding.parameter, frame
            )
            if active:
...
class MiracleFrameExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        self._registry = registry
        self._evaluator = MiracleEvaluator(registry, table)
...
__all__ = ["MiracleEvaluator", "MiracleFrameExecutor"]