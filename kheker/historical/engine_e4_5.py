...
运行: .venv/bin/python kheker/historical/engine_e4_5.py [--fast]
...
def _engine(fast=False):
    from olam.generation.climate import ClimateZone as CZ
    wt = WorldTree()
    clock = WorldClock()
...
def _e5(fast=False):
    ...
    诊断当前 CRN 地址纪律：
      1. 全 olam 包静态扫描：模拟路径零裸 random/np.random——
    ...
    import pathlib
    import re as _re
    pkg_root = pathlib.Path(olam.__file__).resolve().parent
    flag = _re.compile(
