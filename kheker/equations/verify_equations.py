...
运行: .venv/bin/python kheker/equations/verify_equations.py [--fast]

判据：
  V0 生成物漂移：equations.json 与生产世界声明一致、
      impl_digests.json 与生产声明重算一致，且
      GenDeclarationData.lean 与 equations.json + olam/constants.py 真值一致；
  ...
  V3 引擎符合性（olam.adapters.weather.derive）：
...
from olam.constants import LATITUDE_MAX, LATITUDE_MIN, LATITUDE_T_MAX, LATITUDE_T_MIN, SEASONAL_AMP_MAX, SEASONAL_AMP_MIN, SEASONAL_AMP_R_BONUS, SEASONAL_AMP_R_REF, SEASONAL_AMP_T_MAX, SEASONAL_AMP_T_MIN, TEMP_BOUNDS
from olam.adapters.weather.derive import (  # noqa: E402
    derive_latitude, derive_seasonal_amp, precip_type_for,
)
from olam.modules.ids import (  # noqa: E402
    ANNUAL_RAINFALL,
    ANNUAL_TEMPERATURE,
