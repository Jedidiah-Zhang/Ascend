...
事实源 = ``WorldProgram``（``olam.compile``）；本模块把声明投影为
...
from olam.meta.validate import mechanism_digest
...
SCHEMA_VERSION = 3
DECLARATION_ID = "olam.scalar_formulas"
DECLARATION_VERSION = "1"
SLICE_BOUNDARY = (
    "Generation programs (continent / hydrology / tile / weather field) "
    "are declared in olam/generation/__init__.py with content fingerprints "
...
_WEATHER_DEPS = (
    "olam/modules/weather/equations.py",
    "olam/kernel/fixed.py",
    "olam/kernel/tables.py",
    "olam/kernel/diurnal.py",
    "olam/kernel/frozen_tables.py",
    "olam/constants.py",
    "data/world.json",
)
_WORLDGEN_DEPS = _WEATHER_DEPS + (
    "olam/modules/worldgen/equations.py",
    "olam/modules/worldgen/gen_fixed.py",
...
def project(program: object) -> dict[str, object]:
    ...
    return {
        "comment": (
            ...
            "(olam.compile; modules: worldgen + weather)."
        ),
        "declaration": declaration,
        "edges": edges,
        "exogenous_sources": {},
