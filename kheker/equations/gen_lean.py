...
世界声明生成的 ``kheker/equations/equations.json`` 与 config 常量真值
自动生成 kheker/lean/AscendLean/CausalVerification/GenDeclarationData.lean：
...
运行:
  .venv/bin/python kheker/equations/gen_lean.py            # 生成/刷新
  .venv/bin/python kheker/equations/gen_lean.py --check    # 巡检（CI/本地）
...
HERE = Path(__file__).resolve().parent            # kheker/equations
ROOT = HERE.parents[1]                            # 仓库根
DEFAULT_JSON = HERE / "equations.json"
CONFIG_PY = ROOT / "olam" / "constants.py"
OUT_PATH = (HERE.parent / "lean" / "AscendLean" / "CausalVerification"
            / "GenDeclarationData.lean")

# olam/constants.py 中本管线的常量名单（与 verify_equations.py 判据 V2 同源）
CONFIG_CONSTANTS = [
...
def load_config_constants() -> dict[str, float]:
    """以 sys.path 方式 import olam.constants 取常量真值
    （sys.path 处理与 verify_equations.py 相同）。"""
    sys.path.insert(0, str(ROOT))  # 供 import olam / miskhak
    from olam import constants as asc_config  # noqa: E402
...
def build_content(json_path: Path) -> tuple[str, dict]:
    ...
    add("")
    add("生成器：kheker/equations/gen_lean.py（防漂移机制）")
    add("生成命令：.venv/bin/python kheker/equations/gen_lean.py")
    add("巡检命令：.venv/bin/python kheker/equations/gen_lean.py --check")
    add("巡检接入：kheker/equations/verify_equations.py 主流程 V0 步")
    add("")
    ...
    add("")
    add("修复流程：跑生成命令刷新本文件，再于 kheker/lean 下执行")
    ...
    # ── 第一节：config 常量真值 ─────────────────────────
    add("-- ═══ 第一节 世界常量真值（olam/constants.py）═══")
    add("")
    add("-- 纬度推导 LATITUDE_*")
...
def main() -> int:
    ...
    try:
        if args.check:
            ...
            if diff is not None:
                print(diff)
            print("修复: .venv/bin/python kheker/equations/gen_lean.py"
                  "  然后在 kheker/lean 下重跑 lake env lean / lake build")
            return 1
        ...
        print(f"       来源指纹 olam/constants.py sha256:{meta['cfg_sha']}")
        print(f"       边 {meta['edges']} 条，对账定理 {meta['theorems']} 条")
        print("下一步: cd kheker/lean && ~/.elan/bin/lake env lean "
