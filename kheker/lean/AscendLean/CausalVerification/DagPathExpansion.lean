...
出处：`docs/归档/研究理论/世界基座/02-误差传播与反事实.md` 命题 2.5。

- 误差递推 `e_i = ε_i + Σ_{j ∈ Pa_G(i)} L_{j,i}·e_j`（干预节点 `e_s = 0`）；
- 路径和展开
  `e_t = Σ_{u ∈ Anc(t)\I} ε_u · Σ_{paths u→t} Π_{(a,b)∈path} Λ_{a,b}`
  —— 对所有路径**求和**，而非取最大（汇聚节点处多父误差相加；链情形退化回命题 2.1）；
- 汇聚反例 `X1, X2 → X3`（`L = 1`，`ε₁ = ε₂ = 0.1`）：
...
/-- 汇聚拓扑的节点：X1、X2 无父，X3 以两者为父（02 篇命题 2.5）。 -/
...
加上所有父贡献之和（02 篇命题 2.5 的 Star 特例）。 -/
...
/-- `pathWeight` 的递归方程（u ≠ t 时沿入边展开，02 篇误差递推的镜像）。 -/
