...
生成器：kheker/equations/gen_lean.py（防漂移机制）
生成命令：.venv/bin/python kheker/equations/gen_lean.py
巡检命令：.venv/bin/python kheker/equations/gen_lean.py --check
巡检接入：kheker/equations/verify_equations.py 主流程 V0 步
...
修复流程：跑生成命令刷新本文件，再于 kheker/lean 下执行
