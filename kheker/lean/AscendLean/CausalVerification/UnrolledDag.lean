...
出处：
- `docs/归档/研究理论/工程符号体系.md` §3（时间展开图 `\mathcal G^unroll` 无环）
- C2 判据（时间展开无环；`kheker/acceptance/world_checks.py`）
- `docs/归档/研究理论/第一阶段实施定义.md` §4/§5（阶段声明、空间父模板）
...
**有限索引**：声明、节点与父模板的参数一律以
`Fin n` / `Fin m` 索引（n = 分量数，m = 更新阶段数），而不是自然数；
有界索引把"参数在界内"**编码进类型**，
实例化时可直接判定（`decide`/`native_decide`）。
