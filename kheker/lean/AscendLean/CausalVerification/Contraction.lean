...
文档出处：`docs/归档/研究理论/世界基座/02-误差传播与反事实.md`
...
/-- 除法形式的完整刻画（收缩链 0<L<1）：等价于乘法形态的 iff 两端同除以
    负数 log L，不等号同步翻转后形态不变。
    `log L ≠ 0` 在 0<L<1 下自动成立（log L<0）；若 L>1 则方向翻转为
    τ ≤ log(η/δ₀)/log L，故本刻画按收缩链限定。 -/
