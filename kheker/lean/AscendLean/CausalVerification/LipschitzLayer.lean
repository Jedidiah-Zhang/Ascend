...
/-- **无条件和式 Lipschitz**：真方程只读低坐标（局部性）+ 单父坐标 Lipschitz
    （02 篇逐坐标 Lipschitz 条件的字面语义）⟹ 对任意两输入（无需任何一致性前提）
...
/-! ## 第三节：连接定理 — 真实误差满足递推上界（命题 2.1 的轨迹版） -/

/-- **单节点展开步**（02 篇误差拆解：`|Fh(x) − F(x)| ≤ |fh_ℓ(ẑ) − f_ℓ(ẑ)| + |f_ℓ(ẑ) − f_ℓ(z)|`）：
...
/-- 闭式中干预源项消失的形式化（02 篇 `Anc(t) \ I` 的对应物）：
...
/-- 链上的路径权重退化为乘积（02 篇“链情形每节点单父”）：
    u ≤ n 时从 u 到 n+1 只有唯一路径，`W u (n+1) = Π_{j ∈ Icc u n} L j`
    沿链逐段相乘，即命题 2.1 的 `Π_{m=j+1}^{ℓ} L_m`。 -/
