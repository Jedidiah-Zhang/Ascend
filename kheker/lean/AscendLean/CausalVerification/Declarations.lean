...
出处：`kheker/equations/equations.json`（世界参数声明的单一事实来源）+
`olam/constants.py` 的纬度/季节振幅常量 +
`olam/adapters/weather/derive.py` 引擎入口（经声明程序求值）+
`kheker/equations/verify_equations.py` 判据 V2/V3。
...
/-! ### 特例实例化：config 数值（olam/constants.py 的纬度常量） -/
...
/-! ### 特例实例化：config 数值（olam/constants.py 的季节振幅常量） -/
...
LipschitzLayer.error_recurrence_bound（02 篇命题 2.5 连接命题）要求假设
`hlip : ∀ i j x y, j < i → (∀ k < i, k ≠ j → x k = y k) →
   |f i x − f i y| ≤ adj j i * |x j − y j|`
（单父坐标 Lipschitz，02 篇「Λ_{j,i} 关于父 j」）。
