...
import pytest
from miskhak.events import Event, AffectedParty, WorldTree, EventGraph
...
class TestMemoryBound:
    ...
    def test_publish_beyond_threshold_stays_bounded(self):
        ...
        n = _params["publish_n"]
        bus = WorldTree(max_memory_events=max_n)
    ...
    def test_timestamps_monotonic_after_trim(self):
        ...
        bus = WorldTree(max_memory_events=_params["max_events"])
        n = _params["publish_n"]
...
运行方式:
  ...
  # CI 快速（默认）
  pytest testbench/bench/test_world_tree_stress.py -v

  # 中等压力
  STRESS_SCALE=medium pytest testbench/bench/test_world_tree_stress.py -v

  # 完整压力（30M 事件，预计 5-10 分钟）
  STRESS_SCALE=large pytest testbench/bench/test_world_tree_stress.py -v -s
"""
