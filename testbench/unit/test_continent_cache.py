...
覆盖 olam/generation/continent.py 的 serialize/deserialize 与
olam/generation/generator.py 的磁盘缓存接入：指纹 fail-closed（WC-1.2/
...
class TestContinentSerialize:
    ...
    def test_version_mismatch_returns_none(self):
        ...
        with mock.patch(
            "olam.generation.continent_io.CONTINENT_CACHE_VERSION",
...
class TestWorldGeneratorCache:
    ...
    def test_miss_generates_and_writes_cache(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "olam.generation.continent.ContinentGenerator.generate",
            _fake_generate,
        )
