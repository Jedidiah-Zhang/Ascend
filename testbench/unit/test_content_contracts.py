...
from olam.content.loader import _resolve_content_dir, split_ns_id
from miskhak.i18n import get_default, _resolve_content_dir as _resolve_lang_dir
from olam.generation.climate import ClimateZone, _build_climate_templates
from olam.generation.biome import BiomeType, _build_biome_templates
...
class TestContentDirResolution:
    ...
    def test_dev_layout(self, tmp_path):
        """开发：olam/content/loader.py → 仓库根/data。"""
        here = tmp_path / "olam" / "content" / "loader.py"
        (tmp_path / "data").mkdir()
        assert _resolve_content_dir("data", here) == tmp_path / "data"

    def test_release_stage_root(self, tmp_path):
        """发布：数据配送到舞台根 STAGE/data（上四级）。"""
        here = tmp_path / "STAGE" / "server" / "olam" / "content" / "loader.py"
    ...
    def test_i18n_same_convention(self, tmp_path):
        ...
        here = tmp_path / "STAGE" / "server" / "miskhak" / "i18n.py"
...
class TestLanguageSwitchShared:
    def test_labels_follow_shared_set_lang(self):
        """label_key 由 game 侧 i18n 解析；set_lang 经共享实例全局生效。"""
        i = get_default()
        orig = i.lang
        try:
            i.set_lang("en_US")
            assert i.t(BiomeType.TUNDRA.label_key) == "Tundra"
            assert i.t(ClimateZone.ALPINE.label_key) == "Alpine"
            i.set_lang("zh_CN")
            assert i.t(BiomeType.TUNDRA.label_key) == "苔原"
        finally:
            i.set_lang(orig)
