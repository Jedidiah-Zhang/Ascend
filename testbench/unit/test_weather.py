...
def _find_dry_chunk(engine, now: int, annual: float = 800.0):
    ...
    """
    from olam.modules import ids as m
    from olam.generation.weather_field.field import CH_PRECIPITATION
...
class TestWeatherConstants:
    ...
    def test_field_grid_size_positive(self):
        from olam.constants import WEATHER_FIELD_GRID_SIZE
        assert WEATHER_FIELD_GRID_SIZE > 0

    def test_tile_noise_params_positive(self):
        from olam.constants import WEATHER_FIELD_TILE_NOISE_WAVELENGTH, WEATHER_FIELD_TILE_NOISE_SCALE
    ...
    def test_texture_wavelengths_positive(self):
        from olam.constants import TEXTURE_WAVELENGTH_TEMP, TEXTURE_WAVELENGTH_WIND, TEXTURE_WAVELENGTH_PRECIP
        assert TEXTURE_WAVELENGTH_TEMP > TEXTURE_WAVELENGTH_WIND > \
            TEXTURE_WAVELENGTH_PRECIP > 0

    def test_texture_octaves_positive(self):
        from olam.constants import TEXTURE_OCTAVES, TEXTURE_PERSISTENCE, TEXTURE_LACUNARITY, TEXTURE_DRIFT_RATE
    ...
    def test_feature_block_params_positive(self):
        from olam.constants import FEATURE_BLOCK_SIZE, FEATURE_MAX_RADIUS
    ...
    def test_climate_proxy_params_positive(self):
        from olam.constants import CLIMATE_PROXY_TEMP_WAVELENGTH, CLIMATE_PROXY_RAIN_WAVELENGTH, CLIMATE_PROXY_OCTAVES
    ...
    def test_precip_calibration_params(self):
        from olam.constants import PRECIP_SIGNAL_MIN, PRECIP_SIGNAL_MAX, PRECIP_THRESHOLD_DRY, PRECIP_THRESHOLD_WET, PRECIP_ANNUAL_DRY, PRECIP_ANNUAL_WET, PRECIP_INTENSITY_SCALE
    ...
    def test_seasons_per_year_is_four(self):
        from olam.constants import SEASONS_PER_YEAR
        assert SEASONS_PER_YEAR == 4

    def test_season_length_covers_year(self):
        from olam.constants import SEASONS_PER_YEAR, SEASON_LENGTH
        assert SEASON_LENGTH * SEASONS_PER_YEAR == GAME_YEAR

    def test_diurnal_hours_in_range(self):
        from olam.constants import DIURNAL_PEAK_HOUR, DIURNAL_TROUGH_HOUR
    ...
    def test_perturb_scales_positive(self):
        from olam.constants import TEMP_PERTURB_SCALE, HUMIDITY_PERTURB_SCALE, WIND_PERTURB_SCALE, SUNSHINE_PERTURB_SCALE
    ...
    def test_tier_boundaries_defined(self):
        from olam.constants import TEMP_TIER_BOUNDARIES, HUMIDITY_TIER_BOUNDARIES, WIND_TIER_BOUNDARIES, SUNSHINE_TIER_BOUNDARIES
    ...
    def test_humidity_scales_positive(self):
        from olam.constants import HUMIDITY_DIURNAL_SCALE, HUMIDITY_SEASONAL_SCALE
        assert HUMIDITY_DIURNAL_SCALE > 0
        assert HUMIDITY_SEASONAL_SCALE > 0

    def test_sunshine_perturb_scale_positive(self):
        from olam.constants import SUNSHINE_PERTURB_SCALE
        assert SUNSHINE_PERTURB_SCALE > 0
...
from olam.constants import SEASON_LENGTH_DAYS
...
class TestSeason:
    ...
    def test_season_enum_values(self):
        from olam.adapters.weather.season import Season
    ...
    def test_season_of_day_boundaries(self):
        from olam.adapters.weather.season import Season, season_of
    ...
    def test_day_of_year_wraps(self):
        from olam.adapters.weather.season import day_of_year
        assert day_of_year(1) == 0
        assert day_of_year(360) == 359
        assert day_of_year(361) == 0

    def test_day_of_season(self):
        from olam.adapters.weather.season import day_of_season
    ...
    def test_seasonal_temp_offset_summer_peak(self):
        from olam.adapters.weather.season import Season, seasonal_temp_offset
    ...
    def test_seasonal_temp_offset_periodic_across_year(self):
        from olam.adapters.weather.season import seasonal_temp_offset_for_day
    ...
    def test_seasonal_humidity_offset_summer_peak(self):
        """季节湿度在夏季中点峰值。"""
        from olam.adapters.weather.season import Season, seasonal_humidity_offset
        d = SEASON_LENGTH_DAYS // 2
    ...
    def test_seasonal_humidity_offset_same_sign_as_temp(self):
        """季节湿度偏移与温度偏移同向（夏湿冬干）。"""
        from olam.adapters.weather.season import (
...
class TestDiurnal:
    ...
    def test_diurnal_peak_at_14(self):
        from olam.adapters.weather.diurnal import diurnal_temp_offset
    ...
    def test_hour_of_game_time(self):
        from olam.adapters.weather.diurnal import hour_of_game_time
    ...
    def test_diurnal_humidity_peak_at_2(self):
        """湿度昼夜偏移在 02:00 峰值（逆温）。"""
        from olam.adapters.weather.diurnal import diurnal_humidity_offset
    ...
    def test_diurnal_humidity_inverse_to_temp(self):
        """湿度昼夜偏移与温度昼夜偏移符号相反。"""
        from olam.adapters.weather.diurnal import diurnal_temp_offset, diurnal_humidity_offset
    ...
    def test_daylight_hours_equator_constant(self):
        """赤道全年日照≈12h。"""
        from olam.adapters.weather.diurnal import daylight_hours
    ...
    def test_daylight_hours_equals_sunset_minus_sunrise(self):
        ...
        from olam.adapters.weather.diurnal import daylight_hours, sunrise_hour, sunset_hour
...
class TestTextureField:
    ...
    def test_construct_default(self):
        from olam.generation.weather_field.atmosphere import TextureField
        assert TextureField() is not None

    def test_sample_in_range(self):
        from olam.generation.weather_field.atmosphere import TextureField, CH_TEMPERATURE
    ...
    def test_sample_deterministic(self):
        from olam.generation.weather_field.atmosphere import TextureField, CH_PRECIP
        f = TextureField(seed=42)
...
class TestFeatureField:
    ...
    def test_sample_deterministic(self):
        from olam.generation.weather_field.features import FeatureField
    ...
    def test_synthesis_independent_of_insertion_order(self):
        ...
        """
        from olam.generation.weather_field.features import FEATURE_TYPES, FeatureField
    ...
    def test_front_has_no_event_classes(self):
        ...
        from olam.generation.weather_field.features import FEATURE_TYPES, T_FRONT
...
class TestUnifiedWeatherField:
    ...
    def test_sample_deterministic(self):
        from olam.generation.weather_field.field import UnifiedWeatherField
...
class TestRegionTracker:
    ...
    def _make_tracker(self, signal, climate=None):
        from olam.adapters.weather.region_tracker import RegionTracker
...
class TestSeasonalAmplitude:
    ...
    def test_cold_high_amp(self):
        """低温 → 大振幅。"""
        from olam.adapters.weather.derive import derive_seasonal_amp as _derive_seasonal_amp
...
class TestWeatherEngine:
    ...
    def test_derive_latitude_equator(self):
        """年均温 35°C → 纬度≈0（赤道）。"""
        from olam.adapters.weather.derive import derive_latitude as _derive_latitude
...
class TestTierClassification:
    ...
    def test_classify_temperature_bounds(self):
        from olam.adapters.weather.derive import classify_temperature
    ...
    def test_classify_wind_bounds(self):
        from olam.adapters.weather.derive import classify_wind
    ...
    def test_classify_sunshine_bounds(self):
        from olam.adapters.weather.derive import classify_sunshine
...
class TestSunlightIntensity:
    ...
    def test_classify_sunlight_intensity_bounds(self):
        from olam.adapters.weather.derive import classify_sunlight_intensity
...
class TestWeatherReport:
    ...
    def test_report_sun_azimuth_daily_constant_seasonal(self):
        ...
        from olam.adapters.weather.weather_engine import WeatherEngine
        from olam.adapters.weather.diurnal import sunrise_azimuth, _solar_declination
        wt = WorldTree()
        clock = WorldClock()
        e = WeatherEngine(clock, seed=42, world_tree_arg=wt)
        e.register_chunk(0, 0, _make_baseline(), ClimateZone.TEMPERATE_FOREST, 15.0)
...
class TestGlobalEvents:
    ...
    def test_sunrise_variation_across_seasons(self):
        """日出时间随季节变化（夏季早、冬季晚）。"""
        from olam.adapters.weather.diurnal import sunrise_hour
    ...
    def test_sunset_variation_across_seasons(self):
        """日落时间随季节变化（夏季晚、冬季早）。"""
        from olam.adapters.weather.diurnal import sunset_hour
    ...
    def test_equator_constant_sunrise(self):
        """赤道全年日出≈6:00。"""
        from olam.adapters.weather.diurnal import sunrise_hour, sunset_hour
        for doy in (0, 90, 180, 270):
