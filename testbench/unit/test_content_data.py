...
Coverage: olam.content.loader.load_content + climate._build_climate_templates +
biome._build_biome_templates/_build_subdiv_configs。
"""

import pytest

from olam.content.loader import load_content
from olam.generation.climate import (
    ClimateZone,
    SeasonalityMode,
    get_climate_template,
    _build_climate_templates,
)
from olam.generation.biome import (
    BiomeType,
    get_template,
    _OCEAN_BIOMES,
    _SUBDIV_CONFIGS,
...
from olam.generation.weather_field.features import (
    FEATURE_TYPES,
    T_FRONT,
    T_STORM,
    T_COLD_SNAP,
...
class TestClimateData:
    ...
    def test_seasonality_sharpness_pair_resolved(self):
        ...
        from olam.generation.climate import SeasonalityMode
        for zone in ClimateZone:
            tmpl = get_climate_template(zone)
    ...
    def test_label_key_resolves_via_i18n(self):
        """world 只暴露 label_key；game 侧 i18n 解析为显示名。"""
        from miskhak.i18n import get_default

        assert ClimateZone.EQUATORIAL_RAINFOREST.label_key == "climate.equatorial_rainforest"
        assert get_default().t(ClimateZone.ALPINE.label_key) == "高山"
...
class TestBiomeData:
    ...
    def test_label_resolves_via_i18n(self):
        ...
        assert i18n.t(BiomeType.COLD_OCEAN.label_key) == "冷水海洋"
