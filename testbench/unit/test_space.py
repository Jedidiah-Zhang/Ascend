...
import pytest
from olam.constants import MOISTURE_TILE_FREQUENCY, WATER_WADE_COST
from olam.content.chunk import ChunkData, TILE_MAP_SIZE
from olam.content.tile_grid import TileGrid
from olam.generation.biome import BiomeTemplate, BiomeType, TerrainBias, biome_from_attrs, biome_membership, get_template
from olam.generation.climate import ClimateTemplate, ClimateZone, SeasonalityMode, WeatherParams, annual_baseline, classify, get_climate_template
from olam.generation.noise import PerlinNoise
from olam.generation.terrain import TerrainDef, TerrainType, fertility, get_terrain_def, is_buildable, is_passable, movement_cost, water_passability
from olam.content.state_defs import STATE_TYPES, StateConfig, StateParams, state_keys
...
class TestClimateZone:
    ...
    def test_label_key(self):
        """气候档位携带 label_key；显示名由 game 侧 i18n 解析（文本不归 world）。"""
        from miskhak.i18n import get_default

        i18n = get_default()
        assert ClimateZone.ALPINE.label_key == "climate.alpine"
        assert ClimateZone.EQUATORIAL_RAINFOREST.label_key == "climate.equatorial_rainforest"
        assert i18n.t(ClimateZone.ALPINE.label_key) == "高山"
        assert i18n.t(ClimateZone.POLAR_TUNDRA.label_key) == "极地苔原"
    ...
    def test_climate_constants_injection(self):
        ...
        """
        from olam.generation.hydrology import (
    ...
    def test_sea_level_temperature_range(self):
        ...
        from olam.generation.climate import sea_level_temperature
...
class TestBiome:
    ...
    def test_biome_type_label_keys(self):
        """群系类型携带 label_key；显示名由 game 侧 i18n 解析。"""
        from miskhak.i18n import get_default

        i18n = get_default()
        assert BiomeType.TROPICAL_RAINFOREST.label_key == "biome.tropical_rainforest"
        assert BiomeType.COLD_OCEAN.label_key == "biome.cold_ocean"
        assert i18n.t(BiomeType.TROPICAL_RAINFOREST.label_key) == "热带雨林"
        assert i18n.t(BiomeType.TEMPERATE_DECIDUOUS_FOREST.label_key) == "温带落叶林"
    ...
    def test_all_biomes_have_terrain_bias(self):
        ...
        from olam.generation.biome import TerrainBias
        for bt in BiomeType:
            t = get_template(bt)
