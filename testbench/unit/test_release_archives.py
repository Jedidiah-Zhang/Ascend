"""游戏包归档脚本的隔离校验（无 dpkg/rpmbuild 时以替身驱动）。
...
SKIP_WIN = pytest.mark.skipif(os.name != "posix", reason="POSIX 打包脚本")
...
grep -q '^Version: 0.0.3~alpha.1$' "$pkg/DEBIAN/control" || { echo "bad version" >&2; exit 5; }
mkdir -p "$(dirname "$out")"
: > "$out"
"""
...
out="${top#_topdir }/RPMS/x86_64/ascend-0.0.3-0.alpha.1.x86_64.rpm"
mkdir -p "$(dirname "$out")"
: > "$out"
"""
...
def _fixture(tmp_path: Path) -> dict:
    ...
    stage = build / "work" / "staging" / "miskhak-linux"
...
def test_deb_script_packages_stage_with_data_and_lang(tmp_path):
    ...
    script = tmp_path / "build" / "package" / "miskhak" / "linux" / "make_deb.sh"
    result = subprocess.run(["bash", str(script)], cwd=tmp_path,
                            env=fx["env"], capture_output=True, text=True)
    assert result.returncode == 0, result.stderr
    assert (tmp_path / "build" / "dist" / "miskhak" / "ascend-game-linux.deb").exists()
...
def test_rpm_script_packages_stage_with_data_and_lang(tmp_path):
    ...
    script = tmp_path / "build" / "package" / "miskhak" / "linux" / "make_rpm.sh"
    result = subprocess.run(["bash", str(script)], cwd=tmp_path,
                            env=fx["env"], capture_output=True, text=True)
    assert result.returncode == 0, result.stderr
    ...
    assert (tmp_path / "build" / "dist" / "miskhak" / "ascend-game-linux.rpm").exists()
