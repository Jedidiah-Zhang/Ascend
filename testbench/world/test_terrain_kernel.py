...
from olam.content.state_defs import build_param_tables as space_param_tables
from olam import LatticeField, Schedule, WorldProcess, WorldSpec, compile_world
from olam.modules import terrain
from olam.modules.terrain import kernel
from olam.modules.terrain.core import TerrainCore
from olam.modules.terrain.data import build_param_tables
...
class TestTerrainCoreAdapter:
    ...
    def _array_run(self, case: dict) -> dict[str, list[int]]:
        ...
        from olam.modules.terrain.kernel import evolve_accelerated_into

        states = {
            key: array("B", case["states"][key])
            for key in kernel.STATE_KEYS
        }
        evolve_accelerated_into(
            states,
