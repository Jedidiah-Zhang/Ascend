...
from miskhak.save.lineage import (GAME_TIME_KEY, LIVE_ORIGIN_KEY, PARENT_KEY,
                                 SEQ_KEY, SNAPSHOTS_KEY,
                                 parse_snapshot_entries)
from miskhak.save.manifest import parse_seed, seed_to_hex
...
def make_save_handlers(save_manager, game_engine=None):
    ...
    def handle_save_snapshot(msg: dict) -> dict:
        ...
        引擎可用时走 game_engine.snapshot_current()；纯磁盘模式直接打包。
        返回晋升后的保存节点文件名。
        """
        payload = _payload(msg)
