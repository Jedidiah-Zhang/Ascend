...
与 Python 端 miskhak/config.py 保持相同的值，两边需同步更新。
"""
class_name Config
...
const PROTOCOL_VERSION: int = 0x01  # 与后端 miskhak/net/protocol.py 同步
...
const BACKEND_SCRIPT_REL: String = "miskhak/run_server.py"
...
# 游戏时间常量（与后端 miskhak/config.py 同步）
