...
func _backend_source(rel: String) -> String:
	var abs: String = ProjectSettings.globalize_path("res://..").path_join(rel)
...
func test_protocol_version_synced() -> void:
	"""Config.PROTOCOL_VERSION 与 miskhak/net/protocol.py 一致。"""
	var src := _backend_source("net/protocol.py")
...
func test_default_port_synced() -> void:
	...
	var src := _backend_source("net/server.py")
...
func test_default_host_synced() -> void:
	...
	var src := _backend_source("config.py")
...
func test_max_message_size_synced() -> void:
	...
	var py := _backend_source("config.py")
