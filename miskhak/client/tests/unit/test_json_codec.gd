...
func test_encode_decode_roundtrip_dict() -> void:
	...
	var encoded: PackedByteArray = JsonCodec.encode(original)
	var decoded: Variant = JsonCodec.decode(encoded)
...
func test_encode_decode_roundtrip_numbers() -> void:
	assert_eq(JsonCodec.decode(JsonCodec.encode(0)), 0.0)
	assert_eq(JsonCodec.decode(JsonCodec.encode(42)), 42.0)
	assert_eq(JsonCodec.decode(JsonCodec.encode(-7)), -7.0)
	assert_eq(JsonCodec.decode(JsonCodec.encode(3.14)), 3.14)


func test_decode_empty_returns_null() -> void:
	assert_null(JsonCodec.decode(PackedByteArray()))


func test_encode_returns_packedbytearray() -> void:
	var result: PackedByteArray = JsonCodec.encode({"test": 1})
...
func test_decode_invalid_json_returns_null() -> void:
	...
	var result: Variant = JsonCodec.decode(invalid_bytes)
	assert_null(result)
