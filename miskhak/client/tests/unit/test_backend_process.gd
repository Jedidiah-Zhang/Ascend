...
const RUN_SERVER_REL: String = "miskhak/run_server.py"
...
func _setup_project() -> void:
	DirAccess.make_dir_recursive_absolute("user://t_proj/miskhak")
...
func test_spawn_script_missing_fails() -> void:
	...
	assert_eq(reasons, ["backend script not found at user://t_proj/miskhak/run_server.py"])
...
func test_force_kill_commands_include_pid_and_patterns() -> void:
	...
	assert_true(cmds.has(["pkill", ["-9", "-f", "user://t_proj/miskhak/run_server\\.py"]]),
...
func test_untracked_kill_commands_cover_dev_and_packaged() -> void:
	...
	assert_true(cmds.has(["pkill", ["-TERM", "-f", "user://t_proj/miskhak/run_server\\.py"]]),
		"未跟踪清理应覆盖开发模式脚本路径")
