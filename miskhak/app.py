...
from miskhak.net.handlers.map_handler import make_map_handlers
from miskhak.net.handlers.terminal_handler import make_terminal_handler
from miskhak.net.handlers.research_handler import make_research_handler
from miskhak.net.handlers.weather_handler import make_weather_handler
from miskhak.net.handlers.player_handler import make_player_handler
from miskhak.net.handlers.entity_handler import make_entity_handlers
from miskhak.net.handlers.save_handler import make_save_handlers
from miskhak.net.handlers.preview_handler import make_preview_handlers
from olam.generation.generator import WorldGenerator
from olam.generation.tile_gen import TileGenerator
from olam.adapters.chunks.climate_lookup import ChunkClimateLookup
from olam.content.chunk import TILE_MAP_SIZE
from olam.generation.generator import compute_gen_fingerprint
from miskhak.save.chunk_store import ChunkStore
from olam.adapters.chunks.chunk_services import (
...
from miskhak.save.manifest import SEED_MAX, seed_to_hex
...
class GameEngine:
    ...
    def start(
        ...
        *,
        ...
        from olam.runtime import FrameScheduler, bind_periods
        ...
        if self._load_state is not None:
            ...
            logger.info(
                "存档状态已恢复: 干预计划 %d 条 / 记录 %d 条, 注入核 %d 个（时间线投影）",
                len(interventions.get("plan") or []),
                len(interventions.get("records") or []),
                self.weather_engine.field.features.injected_count,
            )
    ...
    def _capture_save(self) -> "_SavePayload":
        ...
        Returns:
            载荷（state 可为 None；chunks 无 chunk_store 时为空）。
        """
        state: dict | None = None
        if (
            self.world_id and self.save_manager
            and self.player_service is not None and self.clock is not None
        ):
            state = collect_state(
    ...
    def _save_worker(self) -> None:
        ...
        while self._running.is_set():
            try:
                payload = self._save_queue.get(timeout=0.5)
            except queue.Empty:
                continue
            try:
    ...
    def _run_save(self, payload: "_SavePayload | None" = None) -> None:
        ...
        state 先于 chunk 提交：中途失败得到"state 新于 chunk"的
        **可恢复组合**（读档按积分游标把 chunk 补齐到 state 时刻），
        而不是 chunk 超前于 state 的隐藏状态（其后果无法由 W_t 重算）。

        `payload=None` 时现场捕获（退出/快照的同步路径）。
        无存档位（调试模式）：仍 flush 事件/chunk，跳过 state/manifest。
        世界失效（轨迹作废）时拒绝提交：磁盘保留**最后有效检查点**，
        作废轨迹不得覆盖它（WC-9.2）。
        """
        if self._world_invalidated is not None:
        ...
        if self.chunk_store is not None:
            self.chunk_store.commit_captured(list(payload.chunks))
        if payload.state is not None:
            self._persist_manifest()
    ...
    def _check_world_invalidated(self, clock) -> None:
        ...
        if scheduler is None or self._world_invalidated is not None:
            return
        reason = scheduler.invalidated
        if reason is None:
            return
        self._world_invalidated = reason
        if clock is not None:
            clock.pause()
        logger.critical(
            "世界失效：已停表并停止保存（轨迹作废，WC-9.2）: %s", reason,
        )
