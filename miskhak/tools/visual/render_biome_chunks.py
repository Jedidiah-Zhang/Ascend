...
def _find_diverse_chunks(
    ...
    """
    from olam.generation.biome import biome_from_attrs
...
def render_terrain_comparison(seed: int = 42) -> None:
    ...
    for idx, (cx, cy, biome) in enumerate(chunks):
        ...
        print(f"  生成 chunk ({cx},{cy}) biome={get_default().t(biome.label_key)}...")
        ...
        label_text = f"{get_default().t(biome.label_key)}  alt={chunk.altitude:.0f}m  T={chunk.mean_temp:.1f}°C"
...
def render_biome_membership_band(seed: int = 42) -> None:
    ...
    print(f"  边界: {get_default().t(b1.label_key)} (chunk {cx},{cy}) | {get_default().t(b2.label_key)} (chunk {cx+1},{cy})")

    from olam.generation.biome import biome_membership
    ...
    draw.text((10, _CHUNK_TILE + 8),
              get_default().t(b1.label_key), fill=(255, 255, 255), font=font)
    draw.text((_CHUNK_TILE + 10, _CHUNK_TILE + 8),
              get_default().t(b2.label_key), fill=(255, 255, 255), font=font)
...
def render_biome_overview(seed: int = 42) -> None:
    ...
    for b in sorted(counts, key=lambda x: x.value):
        n = counts[b]
        pct = n / total * 100
        print(f"    {get_default().t(b.label_key):8s}: {pct:5.1f}%")
