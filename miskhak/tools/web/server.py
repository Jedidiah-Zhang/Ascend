...
用法:
    .venv/bin/python miskhak/tools/web/server.py
...
TERRAIN_COLORS: dict[int, tuple[int, int, int]] = {
    ...
    TerrainType.GRAVEL: (139, 139, 139),
    TerrainType.FERTILE_SOIL: (92, 61, 46),
    TerrainType.ROCK: (107, 107, 107),
    TerrainType.WATER: (91, 158, 207),
    TerrainType.MARSH: (74, 107, 58),
    TerrainType.PERMAFROST: (224, 224, 224),
}
...
def _build_biome_grid(gen: WorldGenerator, cx: int, cy: int, w: int, h: int) -> dict:
    ...
    for bt, color in BIOME_COLORS.items():
        legend[int(bt)] = {"label": get_default().t(bt.label_key), "color": color}
...
def _build_climate_grid(gen: WorldGenerator, cx: int, cy: int, w: int, h: int) -> dict:
    ...
    for cz, color in CLIMATE_COLORS.items():
        legend[int(cz)] = {"label": get_default().t(cz.label_key), "color": color}
    return {"grid": grid, "legend": legend}
...
def _build_chunk_detail(gen: WorldGenerator, cx: int, cy: int) -> dict:
    ...
    return {
        "cx": chunk.cx,
        "cy": chunk.cy,
        "biome": {"id": int(chunk.biome), "label": get_default().t(chunk.biome.label_key)},
        "climate_zone": {
            "id": int(chunk.climate_zone),
            "label": get_default().t(chunk.climate_zone.label_key),
        },
...
def _tile_worker(seed: int, cx: int, cy: int, w: int, h: int, mode: str) -> bytes:
    ...
    chunks = [gen.generate_chunk(cx + x, cy + y) for y in range(h) for x in range(w)]
    layers = (
        [c.annual_baseline.altitude for c in chunks],
        [c.annual_baseline.temperature for c in chunks],
        [c.annual_baseline.rainfall for c in chunks],
        [int(c.climate_zone) for c in chunks],
        [int(c.biome) for c in chunks],
    )
    packer = struct.Struct(f"<{w * h}f")
    return b"".join(packer.pack(*layer) for layer in layers)


def _terrain_worker(seed: int, cx: int, cy: int) -> bytes:
    """由生产 TileGenerator 生成单 chunk 的地形 RGBA 像素数据。"""
    gen = _get_generator(seed)
    grid = TileGenerator(seed, gen.ensure_continent()).generate_chunk(cx, cy)
    rgba = bytearray(TILE_MAP_SIZE * TILE_MAP_SIZE * 4)
    for i, terrain in enumerate(grid.raw_data()):
        r, g, b = TERRAIN_COLORS[terrain]
        p = i * 4
        rgba[p] = r
        rgba[p + 1] = g
        rgba[p + 2] = b
