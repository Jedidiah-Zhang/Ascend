...
用法:
    .venv/bin/python -m miskhak.tools.weather_year_sim [--seed=42] [--fraction=1.0]
"""
...
from miskhak.app import GameEngine, INITIAL_CHUNK_RADIUS
...
def run(seed: int, fraction: float) -> dict:
    ...
    for chunk in loaded.values():
        climate_counts[get_default().t(chunk.climate_zone.label_key)] += 1
