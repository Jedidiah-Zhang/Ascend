...
> **实现注记**：本文记录当前实现的做法与背景，不具规范效力。世界语义以《[世界契约](../世界契约.md)》条款为准；若本文与契约冲突，以契约为准。验收检查由 #50 依据契约 WC-11.2 生成。
...
```
research/acceptance/
  run_acceptance.py     入口：逐项执行 + 产物落盘（--json/--check/--mutation）
  checks.py             16 项判据（C0–C2 / W0–W5 / I0–I1 / L3）
...
证据分层（#50）：**L0** 生成完整性 / **L1** 数学有效性 / **L2** 求值一致性 /
**L3** 世界动力学契约（含正控制）。每项结果的 `plain()` 带 `layer` 字段，
CI 产物按此分组统计。

| 判据 | 层 | 内容 | 关键断言 |
| --- | --- | --- | --- |
| C0 | L0 | 声明完整性 | 生产注册表 C0 全字段校验通过（47 节点 / 35 机制） |
| C1 | L0 | 结构最小性 | 64 个功能依赖见证实测重跑通过 |
| C2 | L1 | 时间展开无环 | 3 帧窗口展开 192 条实例边，秩严格上升 |
| C2' | L1 | C2 判别力自检 | **单条件**违规声明（同帧父位于更晚阶段）必须被拒绝，且错误信息不得混入伴生违规 |
| W0 | L2 | 帧内顺序 | 三阶段构造：参考 / 引擎 / 手算逐值一致（3, 6, 6） |
| W1 | L2 | 节点干预与 CRN | 干预值、入边切断、后代传播、非后代不变、参考一致 |
| W2 | L2 | 三类动态干预 | 值(单帧)=[10,11,12] / 值(窗口)=[10,10,11] 手算一致；**结构变体世界**=[100,200,300]（身份不同）；运行内机制替换被拒绝 |
| W3 | L2 | 空间父模板与边界 | **引擎侧**（`spatial_cells`）× 参考解释器 × 手算三层一致；内部扰动响应 (0, ¼, ½, ¼, 0)；**边缘扰动 replicate 期望 (¾,¼,0,0,0)/(0,0,0,¼,¾)**，并显式拒绝"周期环绕" |
| W4 | L3 | 状态充分性 | 存档读档轨迹 vs 不存档轨迹逐位一致（12 采样点，窗口落在注入核出生之后）；**单因子负例**：仅缺干预 / 仅缺注入核 / 全缺各自必须分叉 |
| W5 | L3 | 观测隔离 | 主体载荷**只能经 `observe` 产生**：键 = 授予可见集、量化生效、两份映射可区分、只含标量 |
| W5' | L3 | W5 判别力自检 | 空观测 / 越权+泄露两种破坏必须让 W5 判红 |
| L3-历史 | L3 | 时间线不可改写历史 | 撤销后同帧重查逐位一致、历史前缀只追加、撤销后新帧不再命中（WC-6.2） |
| L3-CRN | L3 | CRN 配对 | 同种子双引擎：未干预 chunk 逐位相同，干预 chunk 两臂互异且等于干预值（WC-7.3） |
| L3-状态 | L3 | 状态闭合审计 | 每条归属解析到真实载荷/数据库目标；地形非默认状态经捕获、提交、关闭重开后逐值一致；派生项有重算法则、外部输入不入状态；漏映射与缺落盘字段负例报红（WC-3） |
| I0 | L3 | 观测等价、干预可分 | 两候选世界观测分布相同，`do(A=0)` 下 B 的分布不同 |
| I1 | L3 | 分布预测与配对效应 | CRN 配对差恒为 1（4 个 ω） |

**关于 W2 的结构变体**：运行内机制替换已废除（WC-1.3）——W2 现为
"值(单帧)/值(窗口) 两臂 + **结构变体世界**（`w2_variant_registry`：声明 id
与函数不同 → 世界身份不同、轨迹互异且参考=引擎对拍）+ 替换路径关闭
负例"。规律变化的正式研究形态是世界族（WC-1.4：同契约、仅结构/参数不同）。
...
.venv/bin/python research/acceptance/run_acceptance.py --mutation # 生产实现变异探针
```

退出码 0 = 全部通过。CI（`.github/workflows/lean_action_ci.yml`）的
`acceptance` job 依次跑 **`--check`（Lean 实例漂移）**、**全部判据**与
**`--mutation`（变异探针）**，任一失败即红，并上传 `acceptance.json` 产物。
watch 路径已扩展到 `research/acceptance/**`、`backend/ascend/{causal,save}/**`、
`backend/ascend/game.py`、`backend/tests/**` 等。

**产物 manifest（#50）**：`--json` 产物含 `manifest` 段——契约版本、
注册表声明 ID/摘要、世界程序身份与设置、实现摘要表摘要、`git rev-parse HEAD`、
Python/平台；逐判据的种子与输入在其 `result.input`。实验档案据此绑定
"哪个世界、哪份代码"。

**变异探针（#50）**：`--mutation` 分别破坏一处生产实现——`observe` 不量化、
`leaks_research_truth` 恒真、空间边界算子换"周期环绕"、注入核投影置空——
对应判据（W5 / W5 / W3 / W4）必须变红；任一未被检出即失败。
...
- 研究侧回归：`.venv/bin/python -m unittest discover -s research/acceptance -p 'test_*.py'`，覆盖变异探针恢复静态方法、重复运行无污染，以及映射保留但字段缺失/状态段丢失的拒绝判据。
- `tests/unit/test_acceptance_runner.py`：切片自检（C0/C1）、参考解释器
  手算对拍（W0/W2/W3）、首分歧定位、16 项判据全部可执行且报告 JSON 往返、
  关键判据的判别力（W2 三臂互异、W3 边缘响应拒绝环绕、W4 单因子负例、
  W5 键=授予集与 W5′ 空观测/越权泄露自检、L3 漏映射负例）。
- 变异探针：`run_acceptance.py --mutation`（CI 接入）——四处生产实现
  破坏必须被 W5/W3/W4 判据检出。
