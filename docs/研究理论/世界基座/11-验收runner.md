...
| W3 | 空间父模板与边界 | 五格单位扰动响应 (0, ¼, ½, ¼, 0)；边界 replicate 正确 |
| W4 | 状态充分性 | 存档读档后干预读数恒定、注入核恢复 |
| W5 | 观测隔离 | 同一状态 + 两份主体观测：可区分、为研究视图真子集、无研究真值字段 |
...
协议都被授予了全部节点——"按协议收窄可见集"的观测映射层尚未实现。
判据用显式主体观测面（天气读出 + 季节）验证隔离**性质**：两份观测可
区分、主体视图是研究视图的真子集、载荷不含方程版本/父值/随机地址/干预。
观测映射层与智能体接入时，本判据换成按声明派生 $G^i$。
...
退出码 0 = 全部通过。CI（`.github/workflows/lean_action_ci.yml`）新增
`acceptance` job：任一判据失败即红，并上传 `acceptance.json` 产物。
watch 路径已扩展到 `research/acceptance/**`、`backend/ascend/save/**`、
