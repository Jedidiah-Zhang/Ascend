...
取样）。**清单对齐**：``compute_gen_fingerprint``（``olam/generation/generator.py``）
列出的源码文件与配置常量必须被四个程序完整覆盖——任何新增文件/常量
未归属即红（漂移测试 ``testbench/world/test_generation.py``）。
...
from olam.kernel import digest_object, file_digest
...
GENERATION_PROGRAMS: tuple[GenerationDecl, ...] = (
    GenerationDecl(
        ...
        source_files=(
            "olam/generation/continent.py",
            "olam/generation/continent_data.py",
            "olam/generation/noise.py",
            "olam/generation/_perlin.c",
        ),
        constants=(
    ...
    GenerationDecl(
        ...
        source_files=(
            "olam/generation/hydrology.py",
            "olam/generation/streamlines.py",
            "olam/generation/climate.py",
            "olam/generation/_hydrology.c",
            "olam/generation/_streamlines.c",
        ),
        constants=(
        ...
        sampling=(
            "固定大陆场重复生成 → 逐位一致",
            "同一点重复取样 → 逐位一致",
            "内核对（C ↔ Python）逐位一致（testbench/unit/test_hydrology.py）",
        ),
    ...
    GenerationDecl(
        ...
        source_files=(
            "olam/generation/tile_gen.py",
            "olam/content/tile_grid.py",
            "olam/generation/biome.py",
            "olam/generation/terrain.py",
            "olam/generation/noise.py",
            "olam/generation/_perlin.c",
        ),
        constants=(
    ...
    GenerationDecl(
        ...
        source_files=(
            ...
            "olam/generation/weather_field/atmosphere.py",
            "olam/generation/weather_field/features.py",
            "olam/generation/noise.py",
            "olam/generation/_perlin.c",
        ),
        constants=(
