...
def _worldgen_program():
    ...
    if _WORLDGEN_PROGRAM is None:
        ...
        from olam.modules import worldgen
...
def sea_level_temperature(latitude_noise: float) -> float:
    ...
    """
    from olam.modules.ids import LATITUDE_NOISE, SEA_LEVEL_TEMPERATURE
...
def apply_lapse_rate(sea_level_temp: float, altitude: float) -> float:
    ...
    Raises:
        ValueError: 输入越出节点声明值域（fail-closed）。
    """
    from olam.modules.ids import (ALTITUDE, ANNUAL_TEMPERATURE,
                             SEA_LEVEL_TEMPERATURE)
...
def rainfall_from_noise(rainfall_noise: float) -> float:
    ...
    Raises:
        ValueError: 输入越出节点声明值域（fail-closed）。
    """
    from olam.modules.ids import ANNUAL_RAINFALL, RAINFALL_NOISE
...
def classify(
    ...
    ``kheker/equations/reference_check._TOLERANCES``。纯函数，线程安全。
    ...
    Raises:
        ValueError: 输入越出节点声明值域（fail-closed）。
    """
    from olam.modules.ids import (ALTITUDE, ANNUAL_RAINFALL, ANNUAL_TEMPERATURE,
                             CLIMATE_ZONE)
...
def annual_baseline(
    ...
    *,
    ...
    Returns:
        年均基线 WeatherParams。
    """
    from olam.modules.ids import (BASELINE_HUMIDITY, BASELINE_WIND_SPEED,
