...
用法:
    from olam.generation.hydrology import compute_d8, flow_accumulation, erode
...
from olam.generation._cext import load_c_extension
...
# 气候常量注入（单一事实源 = olam/constants.py）
...
def apply_config_climate_constants() -> None:
    """以当前世界常量值注入气候常量（导入期与测试重置用）。"""
    from olam.constants import LAPSE_RATE, RAINFALL_MIN, RAINFALL_MAX, ALPINE_ALTITUDE, POLAR_TEMP, DESERT_RAINFALL, STEPPE_RAINFALL, STEPPE_MIN_TEMP, TROPICAL_TEMP, TEMPERATE_TEMP, RAINFOREST_RAINFALL, TAIGA_RAINFALL
...
def classify_climate_c(temp: float, rainfall: float, altitude: float) -> int:
    ...
    判定阈值由 olam/constants.py 经导入期注入（本模块加载即注入），
...
def erode(
    ...
    *,
    ...
    海洋边界（侵蚀基准面 = 海平面）：海底格（dem<0）不参与
    侵蚀与沉积；陆地格下游为海洋时，侵蚀物质入海不再搬运。
    海底保持生成原貌。
