...
**驱动层设施**（零世界语义，foundation 级）：引擎把"帧内暂存、边界发布"
的记录/缓存写入交给它；世界语义的状态容器是 ``olam/runtime/state.py``
的 ``StateStore``（槽位 + 滞后历史）。事务边界由调度器（``driver.py``）
与各更新点自己声明。
