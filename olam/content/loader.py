...
存成 JSON 文件：改内容只改数据文件、不用改代码。
- `data/<kind>.json`：仓库内置默认内容（单一事实源，入库）。
- 类型校验/缺省由各领域模块（如 olam/generation/terrain.py 的 TerrainDef）负责。

路径解析（双布局回退，与 i18n.py 同源约定）：
- 以 `olam` 包目录为锚，向外查找最近的 `data/`：
  开发=仓库根、发布=舞台根或 `server/` 内；与模块深度无关。
"""
...
def _resolve_content_dir(dirname: str, here: Path | None = None) -> Path:
    ...
    pkg_dir = next((p for p in here.parents if p.name == "olam"), here.parent)
    # 从 olam 包目录向外找最近的 `dirname/`：
