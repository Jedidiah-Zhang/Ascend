...
- 阈值比较在量化域进行——与 float 比较的差异 ≤ 输入量化误差
  （2⁻³¹ 量级）才能翻转，随机制容差登记（见
  ``kheker/equations/reference_check.py::_TOLERANCES``）。
...
from olam.constants import ALPINE_ALTITUDE, DESERT_RAINFALL, LAPSE_RATE, POLAR_TEMP, RAINFOREST_RAINFALL, RAINFALL_MAX, RAINFALL_MIN, STEPPE_MIN_TEMP, STEPPE_RAINFALL, TAIGA_RAINFALL, TEMPERATE_TEMP, TROPICAL_TEMP
